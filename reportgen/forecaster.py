...
# Разбивка по проектам (подтверждена пользователем), точное совпадение кода:
#   ОФД    — п453 (БЕЗ дочернего п45302)
#   Маркет — п470 и п47101
PRODUCTS = {
    "Маркет": {"projects": ["п470", "п47101"], "keys": ["контур.маркет", "маркет"]},
    "ОФД": {"projects": ["п453"], "keys": ["контур.офд", "офд"]},
}
