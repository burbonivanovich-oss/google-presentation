...
def build(template_path, out_path):
    ...
    two_numbers(prs, "Маркет — выполнение плана 2026",
                ("119,0 млн ₽", "Годовой план (Продукт = Маркет).\nФакт за 5 мес — 44,5 млн (91% плана периода), −21% год к году."),
                ("86 %", "Прогноз выполнения года: 102 млн ₽ (база).\nКоридор сценариев 78–91%. Вывод: недовыполнение."))
    ...
    two_numbers(prs, "ОФД — выполнение плана 2026",
                ("37,5 млн ₽", "Годовой план (проект п453).\nФакт за 5 мес — 18,9 млн (123% плана периода), −16% год к году."),
                ("115 %", "Прогноз выполнения года: 43,3 млн ₽ (база).\nКоридор сценариев 103–124%. Вывод: перевыполнение."))
    ...
    # ── Реклама ── (одна сравнительная таблица вместо двух сеток цифр)
    divider(prs, "Контекстная реклама")
    table_slide(prs, "Контекстная реклама: Маркет vs ОФД (янв 2025 – май 2026)",
